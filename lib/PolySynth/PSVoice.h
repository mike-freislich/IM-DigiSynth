...
public:
    ...
    PSVoice(String name, PSComponent *parent = nullptr) : PSComponent(name, parent)
    {
        ...
        aenv_lvl = addParameter(F("A-Env Level"), 1, 0, 1, &voiceAENVdc, target_amplitude);
        penv_lvl = addParameter(F("P-Env Level"), 1, 0, 1, &voicePENVdc, target_amplitude);
        fenv_lvl = addParameter(F("F-Env Level"), 1, 0, 1, &voiceFENVdc, target_amplitude);
        xmod1 = addParameter(F("X-Mod 1->2"), 1, 0, 1, &osc1XMODamt, target_amplitude);
        xmod2 = addParameter(F("X-Mod 2->1"), 1, 0, 1, &osc2XMODamt, target_amplitude);
        ...
        shaperShape = addParameter(F("Shaper wave"), 1, 0, 27, &waveshape1, [](AudioStream *a, float v)
